...
3. **Cross-Dataset Analysis**: Integrate and compare results across M4 (all periods), Traffic, and Weather datasets. Identify configurations that generalize well vs. dataset-specific winners.
...
- Read existing Jupyter notebooks in the repository for analysis patterns and conventions
...
The notebook is an **interpretive artifact** — it communicates what the data means, not merely what it says. You have full autonomy over its structure and content. The analysis scripts (e.g., `vae2_study_analysis.py`) are tools for generating statistics; they are NOT templates the notebook must follow.
...
- Include: executive summary, detailed rankings, statistical tests, visualizations description, interpretation, and next steps
...
- Before creating a SKILL, review existing SKILLS and CLAUDE.md for similar recommendations. Update/replace outdated ones rather than duplicating
- Prefer creating SKILLS over modifying CLAUDE.md. Keep CLAUDE.md concise
- A SKILL should be actionable: someone reading it should know exactly how to configure N-BEATS for that dataset
...
- `sum_losses=True` adds backcast reconstruction loss — encourages full input reconstruction.
...
- Prior recommendations that were confirmed or overturned by new results
