...
- **`blocks/blocks.py`** — All block implementations (~1086 lines, the largest file). Two parallel inheritance hierarchies:
  - `RootBlock(nn.Module)` — Standard backbone: 4 FC layers with activation. Parent of `Generic`, `BottleneckGeneric`, `Seasonality`, `Trend`, `AutoEncoder`, `VAE`, `GenericAEBackcast`, `WaveletV3`, and concrete wavelet subclasses.
