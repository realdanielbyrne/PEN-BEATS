...
\paragraph{Canonical CSVs (\texttt{experiments/results/m4/}).} Paper-sample (step-paper LR): \texttt{comprehensive\_m4\_paper\_sample\_results.csv}, \texttt{comprehensive\_m4\_paper\_sample\_sym10\_fills\_results.csv}, \texttt{tiered\_offset\_m4\_allperiods\_paperlr\_results.csv}, \texttt{m4\_hourly\_sym10\_tiered\_offset\_paperlr\_results.csv}. Paper-sample (plateau LR): \texttt{comprehensive\_m4\_paper\_sample\_plateau\_results.csv}, \texttt{tiered\_offset\_m4\_allperiods\_results.csv}, \texttt{m4\_hourly\_sym10\_tiered\_offset\_results.csv}. Sliding: \texttt{comprehensive\_sweep\_m4\_results.csv}.

\paragraph{Canonical analysis.} Meta-leaderboard at \texttt{experiments/analysis/analysis\_reports/m4\_overall\_leaderboard\_2026-05-03.md}, regenerable from \texttt{experiments/analysis/scripts/m4\_overall\_leaderboard.py}. The strict health filter \texttt{diverged $\lor$ smape$>$100 $\lor$ (best\_epoch$=$0 $\land$ smape$>$50) $\lor$ smape NaN} is applied to every cell.

\paragraph{Canonical YAML configs (\texttt{experiments/configs/}).} Paper-sample faithful: \texttt{comprehensive\_m4\_paper\_sample.yaml}. Paper-sample plateau: \texttt{comprehensive\_m4\_paper\_sample\_plateau.yaml}. Sliding: \texttt{comprehensive\_sweep\_m4.yaml}. Tiered: \texttt{tiered\_offset\_m4\_allperiods.yaml} (and \texttt{m4\_hourly\_sym10\_tiered\_offset.yaml} for the dedicated Hourly file).
...
\subsection{\texttt{active\_g}: per-period and per-dataset}
\label{app:ablations:active_g}

The \texttt{active\_g} extension is a Mann--Whitney win on Hourly (where it is decisive) and a Mann--Whitney loss on every other period. On small-data Milk it eliminates a 63\% bimodal failure mode but pays an absolute sMAPE cost on the converged subset.

\begin{table}[h]
\centering
\small
\caption{\texttt{active\_g} effect by dataset, paired Mann--Whitney. ``Quality cost'' is the absolute sMAPE delta on the converged subset of seeds; the convergence column is the failure-rate reduction.}
...
M4-Yearly       & $13.616$ ($n{=}191$) & $13.692$ ($n{=}200$) & $+0.076$ & $<10^{-4}$  & $4.5\% \to 0\%$ stuck \\
...
\paragraph{Per-period verdict on M4 paper-sample.} Hourly: $9/9$ matched pairs improve under \texttt{agf} (5 significant at $p<0.05$); Hourly Δ in the canonical leaderboard is $-0.148$ sMAPE (\texttt{NBEATS-IG\_30s\_agf} $8.758$ vs.\ \texttt{ag0} $8.906$). Yearly/Quarterly/Monthly/Weekly/Daily: \texttt{agf} ties or loses on every protocol-LR cell tested. Treat \texttt{active\_g='forecast'} as a Hourly-only intervention.
...
\midrule
1 & \texttt{T+DB3V3\_10s\_tiered\_agf}      & step    & $\mathbf{13.486 \pm 0.086}$ & 5.07M  & 5 \\
2 & \texttt{T+Coif2V3\_30s\_bdeq}           & plateau & $13.542 \pm 0.148$          & 15.24M & 10 \\
3 & \texttt{TWAE\_10s\_ld32\_ag0}           & plateau & $13.546 \pm 0.102$          & 0.48M  & 10 \\
4 & \texttt{TALG+DB3V3ALG\_10s\_ag0}        & step    & $13.550 \pm 0.096$          & 1.04M  & 10 \\
5 & \texttt{TWAE\_10s\_ld32\_sym10\_ag0}    & plateau & $13.553 \pm 0.165$          & 0.48M  & 10 \\
\midrule
...
\subsection{M4-Quarterly ($H=8$, $L=40$)}
...
2 & \texttt{TAE+Sym10V3AE\_10s\_tiered}     & step    & $10.330 \pm 0.056$          & 1.06M  & 4 \\
3 & \texttt{TAELG+DB3V3AELG\_10s\_tiered}   & step    & $10.342 \pm 0.050$          & 1.06M  & 4 \\
4 & \texttt{NBEATS-IG\_10s\_agf}            & plateau & $10.354 \pm 0.115$          & 19.64M & 10 \\
5 & \texttt{T+Sym10V3\_30s\_tiered\_agf}    & plateau & $10.356 \pm 0.065$          & 15.34M & 10 \\
\midrule
\multicolumn{6}{l}{\textit{Sliding}} \\
\midrule
...
\subsection{M4-Monthly ($H=18$, $L=90$)}
...
\subsection{M4-Weekly ($H=13$, $L=65$)}
...
\# & Config & LR & sMAPE $\pm$ std & Params & $n$ \\
\midrule
1 & \texttt{T+Sym10V3\_10s\_tiered\_ag0}    & plateau & $\mathbf{3.012 \pm 0.031}$ & 5.25M & 10 \\
2 & \texttt{TAELG+Sym10V3AELG\_10s\_tiered} & plateau & $3.013 \pm 0.023$          & 1.14M & 10 \\
3 & \texttt{TAE+Sym10V3AE\_10s\_tiered}     & step    & $3.015 \pm 0.046$          & 1.14M & 4 \\
4 & \texttt{T+DB3V3\_10s\_tiered\_ag0}      & plateau & $3.023 \pm 0.041$          & 5.25M & 10 \\
5 & \texttt{TAE+DB3V3AE\_30s\_tiered}       & plateau & $3.026 \pm 0.041$          & 3.41M & 10 \\
\midrule
\multicolumn{6}{l}{\textit{Sliding}} \\
\midrule
...
\subsection{M4-Hourly ($H=48$, $L=240$)}
