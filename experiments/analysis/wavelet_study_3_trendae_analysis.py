"""Wavelet Study 3 TrendAE successive-halving analysis across datasets.

Behavior:
- Discovers datasets from run_wavelet_study_3_successive_trendae.WAVELET_STUDY3_DATASETS
- Resolves CSV paths from run_wavelet_study_3_successive_trendae._search_csv_path
...
- Uses llm_commentary pattern (when available) for interpretation sections
"""
...
from run_wavelet_study_3_successive_trendae import (
...
def section_marginals(df, m):
    ...
    for factor in ["wavelet", "bd_label", "ttd", "active_g_cfg"]:
        ...
        if len(grp_raw) >= 2:
            best_val = grp_raw.index[0]
            worst_val = grp_raw.index[-1]
            delta = float(grp_raw.iloc[-1]["med_metric"] - grp_raw.iloc[0]["med_metric"])
            llm_text = None
            if m["has_owa"]:
                llm_ctx = {
                    "parameter_name": factor,
                    "best_value": str(best_val),
                    "best_owa": float(grp_raw.iloc[0]["med_metric"]),
                    "worst_value": str(worst_val),
                    "worst_owa": float(grp_raw.iloc[-1]["med_metric"]),
                    "delta": delta,
                    "all_values": [
                        {"value": str(level), "med_owa": float(row["med_metric"])}
                        for level, row in grp_raw.iterrows()
                    ],
                }
            ...
            if llm_text:
                ...
                print(
                    f"`{best_val}` is best by median {m['primary_label']} "
                    f"with a {delta:.4f} gap to `{worst_val}`.\n"
                )
...
def section_stability(df, m):
    ...
    if last_spread is not None and not last_spread.empty:
        llm_ctx = {
...
def section_progression(df, m):
    ...
    if "Delta" in prog.columns:
        ...
        if llm_text:
            ...
            print(f"{improved}/{len(prog)} finalist configs improved as training budget increased.\n")
...
def section_baseline_comparisons(df, m, dataset_name):
    ...
    if not top.empty:
        best_cfg = top.iloc[0]
        pass_rows = {}
        if "Pass" in top.columns:
            per_pass = top.sort_values("OWA").groupby("Pass").head(1)
            for _, row in per_pass.iterrows():
                pass_rows[str(row["Pass"])] = {
                    "config": str(row["Config"]),
                    "owa": float(row["OWA"]),
                }

        llm_ctx = {
            "variants": ["WaveletV3+TrendAE"],
            "round_results": {
                "best_overall": {
                    "config": str(best_cfg["Config"]),
                    "pass": str(best_cfg["Pass"]),
                    "owa": float(best_cfg["OWA"]),
                },
                "best_by_pass": pass_rows,
                "baselines": {
                    name: {"owa": vals["owa"]} for name, vals in BASELINES.items()
                },
            },
        }
...
def main():
    parser = argparse.ArgumentParser(description="Wavelet Study 3 TrendAE multi-dataset analysis")
    parser.add_argument(
        "--dataset",
        default="all",
    ...
    print("# Wavelet Study 3 TrendAE - Multi-Dataset Analysis\n")
