...
On Milk 10-stack convergence, enabling `active_g` does not improve validation-loss quality. `Milk10_baseline` achieves the lower median best validation loss (1.5428 vs 2.4012), lower median final loss, and a lower overfitting ratio. While `Milk10_activeG` has fewer divergence events in this run set, its optimization endpoint is consistently worse.

Given the magnitude of the loss gap and the non-significant one-sided test in favor of `active_g=True`, this experiment supports baseline output heads for this dataset and stack size.

## 7. Recommended Parameter Configurations

### Primary recommendation

- **Config:** `Milk10_baseline`
- **Setting:** `active_g=False`
- **Median best_val_loss:** 1.5428
- **Median final/best loss ratio:** 1.2168
- **When to use:** default for best validation performance on Milk with this 10-stack setup.

### Secondary recommendation (stability-oriented fallback)

- **Config:** `Milk10_activeG`
- **Setting:** `active_g=True`
- **Median best_val_loss:** 2.4012
- **Median final/best loss ratio:** 1.3024
- **When to use:** only if strict no-divergence behavior is prioritized above best validation loss.
