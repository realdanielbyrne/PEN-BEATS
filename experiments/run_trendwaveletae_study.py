"""
TrendWaveletAE Pure-Stack Study — Successive Halving Search + Cross-Dataset Benchmark

Config-driven architecture search for pure homogeneous stacks of TrendWaveletAE
or TrendWaveletAELG blocks, where wavelet family is a parameter (wavelet_type)
rather than encoded in the block class name.

Scientific question: Does embedding trend + wavelet in one block (pure stack)
...
Common CLI scripts:
    ...
    python experiments/run_trendwaveletae_study.py search \
        --config experiments/configs/trendwaveletae_pure_m4.yaml

    # Search one dataset, a single round only
    python experiments/run_trendwaveletae_study.py search \
        --config experiments/configs/trendwaveletae_pure_traffic.yaml \
        --round 1

    # Analyze/rank existing search results (no new training)
    python experiments/run_trendwaveletae_study.py search \
        --config experiments/configs/trendwaveletae_pure_weather.yaml \
        --round all --analyze

    # Run all 4 dataset searches then cross-dataset benchmark
    python experiments/run_trendwaveletae_study.py all \
        --configs \
        experiments/configs/trendwaveletae_pure_m4.yaml \
        experiments/configs/trendwaveletae_pure_tourism.yaml \
        experiments/configs/trendwaveletae_pure_traffic.yaml \
        experiments/configs/trendwaveletae_pure_weather.yaml

    # Run cross-dataset benchmark only (uses completed round-3 CSVs)
    python experiments/run_trendwaveletae_study.py cross \
        --configs \
        experiments/configs/trendwaveletae_pure_m4.yaml \
        experiments/configs/trendwaveletae_pure_tourism.yaml \
        experiments/configs/trendwaveletae_pure_traffic.yaml \
...
WAVELET_TYPES = [
    "haar", "db2", "db3", "db4", "db10", "db20",
    "coif1", "coif2", "coif3", "coif10",
    "sym2", "sym3", "sym10", "sym20",
]
...
def _configure_process_logging_from_env() -> None:
    log_path = os.environ.get("TRENDWAVELETAE_LOG_PATH")
    if log_path:
...
def canonical_config_id(block_type: str, wavelet_type: str, bd_label: str, trend_dim: int, latent_dim: int) -> str:
    return f"{block_type}|{wavelet_type}|{bd_label}|td{int(trend_dim)}|ld{int(latent_dim)}"


def config_name_from_canonical(canonical_id: str) -> str:
    block_type, wavelet_type, bd_label, td_tag, ld_tag = canonical_id.split("|")
    return f"{block_type}_{wavelet_type}_{bd_label}_{td_tag}_{ld_tag}"
...
def generate_search_configs(study_cfg: StudyConfig, promoted_config_names: set[str] | None = None) -> dict[str, dict]:
    ...
    trend_dims = list(study_cfg.search_space.get("trend_dims") or [])
...
def _search_csv_path(study_cfg: StudyConfig) -> str:
    return os.path.join(
        ...
        study_cfg.dataset,
        study_cfg.output.get("search_csv_filename", "trendwaveletae_pure_study_results.csv"),
    )


def _cross_csv_path(study_cfg: StudyConfig) -> str:
    return os.path.abspath(study_cfg.output.get("cross_csv_path", "experiments/results/trendwaveletae_pure_cross_dataset_results.csv"))
...
def rank_and_promote(
    ...
    for config_name, rs in grouped.items():
        ...
        for r in rs:
            ...
            mp_val = _safe_float(r.get("meta_predicted_best"), default=float("nan"))
            if math.isfinite(mp_val):
                meta_preds.append(mp_val)
    ...
    print(f"  {'=' * 80}")
    print(f"  {'Rank':<6} {'Config':<50} {'ValLoss':>10} {'Meta':>10} {'Div%':>6}")
    print(f"  {'-' * 88}")
    for i, r in enumerate(rankings[: min(40, total)]):
        ...
        print(
            f"  {i + 1:<5}{marker} {r['config_name']:<50} "
...
def _run_single_search_job(
    ...
    extra_row = {
        ...
        "trend_dim_cfg": int(config["trend_dim"]),
    ...
    block_type = config["block_type"]
    experiment_name = f"trendwaveletae_search_r{round_num}"

    run_single_experiment(
        experiment_name=experiment_name,
        config_name=config["config_name"],
        category="trendwaveletae_search",
        ...
        trend_thetas_dim=int(config["trend_dim"]),
...
def _run_search_round_parallel(
    ...
    for config in configs.values():
        for run_idx in range(n_runs):
            if result_exists(
                csv_path,
                f"trendwaveletae_search_r{round_num}",
                config["config_name"],
                study_cfg.period,
                run_idx,
            ):
...
def _canonical_from_row(row: dict) -> str:
    td = _safe_float(row.get("trend_dim_cfg"), default=float("nan"))
    ld = _safe_float(row.get("latent_dim_cfg"), default=float("nan"))
    if not math.isfinite(td) or not math.isfinite(ld):
        return ""
    return canonical_config_id(
        row.get("block_type", ""),
        row.get("wavelet_type", ""),
        row.get("bd_label", ""),
        int(td),
        int(ld),
    )
...
def _parse_canonical(canonical_id: str) -> dict:
    ...
    return {
        ...
        "wavelet_type": wavelet_type,
        "bd_label": bd_label,
        "trend_dim": int(td_tag.replace("td", "")),
...
def _run_single_cross_job(
    ...
    repeats = int(dataset_cfg.architecture.get("repeats", 10))
    config_name = config_name_from_canonical(candidate["canonical_config_id"])

    extra_row = {
        "search_round": "",
        "block_type": block_type,
        "wavelet_type": parsed["wavelet_type"],
        "wavelet_family": parsed["wavelet_type"],
        "basis_dim": int(basis_dim),
        "basis_offset": int(dataset_cfg.training.get("basis_offset", 0)),
        "trend_dim_cfg": int(parsed["trend_dim"]),
    ...
    run_single_experiment(
        experiment_name="trendwaveletae_cross_dataset",
        config_name=config_name,
        category="trendwaveletae_cross_dataset",
        stack_types=[block_type] * repeats,
        ...
        trend_thetas_dim=int(parsed["trend_dim"]),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="TrendWaveletAE pure-stack successive-halving + cross-dataset runner"
    )
...
def main() -> None:
    ...
    if not getattr(args, "no_tmp_log", False):
        ...
        if not log_path:
            ...
            log_path = f"/tmp/trendwaveletae_study_{ts}_{os.getpid()}.log"
        _configure_process_logging(log_path)
        os.environ["TRENDWAVELETAE_LOG_PATH"] = log_path
