...
Architecture:
  M4 / Tourism:             [TrendAE, <WaveletV3>] * 5   (10 stacks total)
  Traffic-96 / Weather-96:  [TrendAE, <WaveletV3>] * 10  (20 stacks total)
...
Usage:
    ...
    python experiments/run_wavelet_study_3_successive_trendae.py --dataset traffic
...
# Datasets that use 20 stacks (10 TrendAE+Wavelet pairs) instead of the default 10
    ...
    for wavelet in SEARCH_WAVELETS:
        ...
        for bd_label, bd_value in basis_dims:
            for trend_td in SEARCH_TREND_THETAS_DIMS:
                ...
                stack_types = ["TrendAE", wavelet] * n_stack_pairs  # 20 or 10 stacks
...
def run_wavelet_study3_search(args):
    ...
    print(f"  Stack pairs: {n_pairs} ([TrendAE, WaveletV3] × {n_pairs} = {n_pairs * 2} stacks)")
    if n_gpus >= 2:
