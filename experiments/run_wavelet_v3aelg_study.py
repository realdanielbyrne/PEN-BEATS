"""
WaveletV3AELG Study — Successive Halving Search + Cross-Dataset Benchmark

Config-driven architecture search for alternating Trend + WaveletV3AELG stacks,
followed by global top-10 cross-dataset validation.

V3AELG blocks use Learned Gate (LG) autoencoders where sigmoid(gate) * z at
the latent bottleneck lets the network discover effective latent dimensionality.
latent_dim is a fixed scalar (not a search axis) — the gate handles sparsity.
...
Common CLI scripts:
    ...
    python experiments/run_wavelet_v3aelg_study.py search \
        --config experiments/configs/wavelet_v3aelg_trendaelg_m4.yaml

    # Search one dataset, a single round only
    python experiments/run_wavelet_v3aelg_study.py search \
        --config experiments/configs/wavelet_v3aelg_trendaelg_traffic.yaml \
        --round 1

    # Analyze/rank existing search results (no new training)
    python experiments/run_wavelet_v3aelg_study.py search \
        --config experiments/configs/wavelet_v3aelg_trendaelg_weather.yaml \
        --round all --analyze

    # Run all 4 dataset searches then cross-dataset benchmark
    python experiments/run_wavelet_v3aelg_study.py all \
        --configs \
        experiments/configs/wavelet_v3aelg_trendaelg_m4.yaml \
        experiments/configs/wavelet_v3aelg_trendaelg_tourism.yaml \
        experiments/configs/wavelet_v3aelg_trendaelg_traffic.yaml \
        experiments/configs/wavelet_v3aelg_trendaelg_weather.yaml

    # Run cross-dataset benchmark only (uses completed round-3 CSVs)
    python experiments/run_wavelet_v3aelg_study.py cross \
        --configs \
        experiments/configs/wavelet_v3aelg_trendaelg_m4.yaml \
        experiments/configs/wavelet_v3aelg_trendaelg_tourism.yaml \
        experiments/configs/wavelet_v3aelg_trendaelg_traffic.yaml \
...
def _configure_process_logging_from_env() -> None:
    log_path = os.environ.get("WAVELET_V3AELG_LOG_PATH")
    if log_path:
        _configure_process_logging(log_path)


def _resolve_latent_dims(raw: dict) -> list[int]:
    """Read latent_dim (scalar) or latent_dims (list) from search_space.

    - ``latent_dim: 16``  →  ``[16]``   (TrendAELG: gate handles sparsity)
    - ``latent_dims: [2, 5, 8]``  →  ``[2, 5, 8]``  (TrendAE: search axis)
    """
    ss = raw.get("search_space") or {}
    ld = ss.get("latent_dim")
    if ld is not None:
        return [int(ld)]
    lds = ss.get("latent_dims")
    if isinstance(lds, list) and len(lds) > 0:
        return [int(x) for x in lds]
    return [16]
...
def load_study_config(path: str) -> StudyConfig:
    ...
    study_name = str(raw.get("study_name", "wavelet_v3aelg_search")).strip()
    ...
    latent_dims = _resolve_latent_dims(raw)

    defaults = {
        "architecture": {
            "trend_block": "TrendAELG",
            "repeats": 5,
        },
        "training": {
            "active_g": False,
        ...
        "search_space": {
            "wavelets": list(WAVELET_V3AELG_FAMILIES),
...
def generate_search_configs(study_cfg: StudyConfig, promoted_config_names: set[str] | None = None) -> dict[str, dict]:
    ...
    latent_dims = list(study_cfg.latent_dims)
...
def _search_csv_path(study_cfg: StudyConfig) -> str:
    return os.path.join(
        ...
        study_cfg.dataset,
        study_cfg.output.get("search_csv_filename", "wavelet_v3aelg_study_results.csv"),
    )


def _cross_csv_path(study_cfg: StudyConfig) -> str:
    return os.path.abspath(study_cfg.output.get("cross_csv_path", "experiments/results/wavelet_v3aelg_cross_dataset_results.csv"))
...
def _run_single_search_job(
    ...
    experiment_name = f"{study_cfg.study_name}_r{round_num}"

    run_single_experiment(
        experiment_name=experiment_name,
        config_name=config["config_name"],
        category=study_cfg.study_name,
...
def _run_search_round_sequential(
    ...
    print(f"\n  {'-' * 64}")
    print(f"  Round {round_num}: {len(configs)} configs x {n_runs} runs x {max_epochs} epochs")
    print(f"  {'-' * 64}")
...
def _run_search_round_parallel(
    ...
    for config in configs.values():
        for run_idx in range(n_runs):
            if result_exists(
                csv_path,
                f"{study_cfg.study_name}_r{round_num}",
                config["config_name"],
                study_cfg.period,
                run_idx,
            ):
...
def _run_single_cross_job(
    ...
    run_single_experiment(
        experiment_name=f"{dataset_cfg.study_name}_cross_dataset",
        config_name=config_name,
        category=f"{dataset_cfg.study_name}_cross",
    ...
    for dataset_name, cfg in sorted(by_dataset.items()):
        ...
        for candidate in global_top10:
            ...
            for run_idx in range(n_runs):
                if result_exists(
                    cross_csv,
                    f"{cfg.study_name}_cross_dataset",
                    config_name,
                    cfg.period,
                    run_idx,
                ):
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="WaveletV3AELG successive-halving + cross-dataset runner"
    )
...
def main() -> None:
    ...
    if not getattr(args, "no_tmp_log", False):
        ...
        if not log_path:
            ...
            log_path = f"/tmp/wavelet_v3aelg_study_{ts}_{os.getpid()}.log"
        _configure_process_logging(log_path)
        os.environ["WAVELET_V3AELG_LOG_PATH"] = log_path
